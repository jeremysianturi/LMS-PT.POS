...
import com.pos.lms.core.data.source.local.entity.roadmap.MCPPromosiEntity
 ...
 */
object DataMapperMCPPromosi {
    fun mapResponsesToEntities(input: List<ECPResponse>): List<MCPPromosiEntity> {
        val list = ArrayList<MCPPromosiEntity>()
        input.map {
            val entity = MCPPromosiEntity(
    ...
    fun mapEntitiesToDomain(input: List<MCPPromosiEntity>): List<MCPPromosi> =
        input.map {
            MCPPromosi(
