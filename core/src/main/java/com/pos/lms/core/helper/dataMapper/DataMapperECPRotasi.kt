...
import com.pos.lms.core.data.source.local.entity.roadmap.ECPRotasiEntity
 ...
 */
object DataMapperECPRotasi {
    fun mapResponsesToEntities(input: List<ECPResponse>): List<ECPRotasiEntity> {
        val list = ArrayList<ECPRotasiEntity>()
        input.map {
            val entity = ECPRotasiEntity(
    ...
    fun mapEntitiesToDomain(input: List<ECPRotasiEntity>): List<ECPRotasi> =
        input.map {
            ECPRotasi(
