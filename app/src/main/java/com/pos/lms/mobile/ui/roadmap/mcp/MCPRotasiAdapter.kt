...
class MCPRotasiAdapter : RecyclerView.Adapter<MCPRotasiAdapter.UserViewHolder>() {
    ...
    var onItemClick: ((MCPRotasi) -> Unit)? = null

    private val mData = ArrayList<MCPRotasi>()

    fun setData(newListData: List<MCPRotasi>?) {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): MCPRotasiAdapter.UserViewHolder {
        val mView =
    ...
    override fun onBindViewHolder(holder: MCPRotasiAdapter.UserViewHolder, position: Int) {
        holder.bind(mData[position])
    }

    inner class UserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val binding = ItemListEcpBinding.bind(itemView)
        fun bind(data: MCPRotasi) {
            with(binding) {
