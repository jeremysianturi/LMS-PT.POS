...
class ECPPromosiAdapter : RecyclerView.Adapter<ECPPromosiAdapter.UserViewHolder>() {
    ...
    private val mData = ArrayList<ECPPromosi>()

    fun setData(newListData: List<ECPPromosi>?) {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): ECPPromosiAdapter.UserViewHolder {
        val mView =
    ...
    override fun onBindViewHolder(holder: ECPPromosiAdapter.UserViewHolder, position: Int) {
        holder.bind(mData[position])
    }

    inner class UserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val binding = ItemListEcpBinding.bind(itemView)
        fun bind(data: ECPPromosi) {
            with(binding) {
